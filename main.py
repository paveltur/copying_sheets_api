...
PDL_NEW_CALL_LIST = os.environ.get("PDL_NEW_CALL_LIST")  # Обзвон отвалившихся
...
def pdl_new_update(history, history_sheet_id, ranges_for_history, call_list, ranges_for_call_list,
    ...
    list_of_row = [row for row in data_2["valueRanges"][0]["values"] if row[0] == "step 7" or row[0] == "decision"]
    ...
    range_body = {"values": list_of_row[index_cut_list:]}  # массив данных для записи
    # номера для последней строки исторических данных, чтобы протянуть формулу статусов на вновь скопированные данные
        ...
        "requests": [
            {
                "copyPaste": {
                    ...
                    "destination": {
                        ...
                        "startColumnIndex": copy_paste_start_column_index,  # столбец по умолчанию H
                        "endColumnIndex": copy_paste_end_column_index  # столбец по умолчанию H
                    },
    ...
    if not range_body["values"]:
        ...
        return ["PDL_NEW", "true_2", add_new_data]
